...
os.system("cp -v CHIKV_master.sqlite CHIKV_batch.sqlite")

logger.debug(f"{hippo.__file__=}")

animal = hippo.HIPPO("CHIKV_batch", "CHIKV_batch.sqlite")
...
def parse_syndirella_products_csv(
    df: pd.DataFrame, product_csv: bool, base_output_path: str
):
    """
    ...
    for i, row in tqdm(df.iterrows()):
        ...
        if product_csv:
            ...
            pose_path = os.path.join(
                str(base_output_path), name, f"{name}.minimised.mol"
            )
        ...
        reactant1 = animal.register_compound(
            smiles=row.r1_smiles, return_compound=False, commit=False
        )

        # r2 reactant could not exist yet
        if "r2_smiles" in row:
            reactant2 = animal.register_compound(
                smiles=row.r2_smiles, return_compound=False, commit=False
            )
        ...
        reaction = animal.register_reaction(
            type=row.reaction, product=product, reactants=reactants, commit=False
        )
...
def parse_syndirella_products_batch(path: str):
    ...
    for i, subdirectory in enumerate(files):
        ...
        product_csvs = [
            c
            for c in product_csvs
            if str(c).endswith("_placements.csv") or not is_last_step(c)
        ]
        ...
        for j, product_csv in enumerate(product_csvs):
            ...
            parse_syndirella_products_csv(df, has_regarded, base_output_dir)
